...
fun SignupScreen(navController: NavController, viewModel: AuthViewModel = viewModel()) {
    ...
    Column(
        ...
        PrimaryButton(
            ...
            enabled = !authState.isLoading && authState.fullNameError == null && authState.emailError == null && authState.passwordError == null && authState.confirmPasswordError == null
        )
        Spacer(modifier = Modifier.height(16.dp))

        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
            Text("Already have an account?")
            TextButton(onClick = { navController.navigate(AppRoute.Login.javaClass.name) }) {
                Text("Login")
            }
        }
        Spacer(modifier = Modifier.height(32.dp))

        Text("Or sign up with:", style = MaterialTheme.typography.labelLarge)
        Spacer(modifier = Modifier.height(16.dp))
        Row(
